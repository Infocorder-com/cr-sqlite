...
// Define the symbol the linker asks for directly rather than declaring a lang item,
// which would require nightly. Both profiles set `panic = "abort"`, so this is never
// actually invoked -- it only has to exist to satisfy the link.
